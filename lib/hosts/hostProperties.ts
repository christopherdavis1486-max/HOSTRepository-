...
export async function listAllAmenities() {
  const result = await db.query(
    `SELECT id, name, slug FROM amenities ORDER BY name`
  );
...
export async function createPropertyForHost(
  ...
  return {
    ...
    cancellationPolicy: row.cancellation_policy_name
      ? {
          ...
          description: row.cancellation_policy_description,
