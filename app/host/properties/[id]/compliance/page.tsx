...
export default function PropertyCompliancePage({params}:{params:Promise<{id:string}>}){
  ...
  const [items,setItems]=useState<Item[]>(blankItems()); const [message,setMessage]=useState(""); const [reviewNote,setReviewNote]=useState(""); const [busy,setBusy]=useState(false);
  useEffect(()=>{params.then(value=>setId(value.id));},[params]);
  useEffect(()=>{if(!id)return;fetch(`/api/host/properties/${id}/compliance`,{credentials:"include",cache:"no-store"}).then(async response=>{const body=await response.json();if(!response.ok){setMessage(body.error?.message??ht("Unable to load compliance."));setLoadState("error");return;}const saved=new Map(body.compliance.items.map((item:Record<string,unknown>)=>[item.category,item]));setName(body.compliance.property.name);setStatus(body.compliance.property.compliance_status);setReviewNote(String(body.compliance.property.compliance_review_note??""));setItems(blankItems().map(item=>{const row=saved.get(item.category) as Record<string,unknown>|undefined;return row?{category:item.category,applicability:row.applicability as Item["applicability"],ownerDeclaredCompliant:Boolean(row.owner_declared_compliant),evidenceUrl:String(row.evidence_url??""),evidenceReference:String(row.evidence_reference??""),validUntil:String(row.valid_until??"").slice(0,10),ownerNote:String(row.owner_note??""),reviewStatus:String(row.review_status??""),reviewerNote:row.reviewer_note as string|null}:item;}));setLoadState("ready");}).catch(()=>{setMessage(ht("Unable to load compliance."));setLoadState("error");});},[id,ht]);
  ...
  if(loadState!=="ready") return <main style={{minHeight:"100vh",background:"#100f0c",color:"#fff8e8",padding:"28px 6vw 80px",fontFamily:"Arial,sans-serif"}}><div style={{maxWidth:980,margin:"0 auto"}}><a href="/host/properties" style={{color:"#c4a98b"}}>← {ht("Back to Properties")}</a><p role={loadState==="error"?"alert":"status"} style={{marginTop:72,color:loadState==="error"?"#ef6b5b":"#c4a98b"}}>{loadState==="error"?message:ht("Loading compliance…")}</p></div></main>;
  return <main style={{minHeight:"100vh",background:"#100f0c",color:"#fff8e8",padding:"28px 6vw 80px",fontFamily:"Arial,sans-serif"}}><div style={{maxWidth:980,margin:"0 auto"}}>
    ...
    <section style={card}><p><strong>{ht("Status")}:</strong> {hStatus(status)}</p><p style={{color:"#c4a98b",lineHeight:1.6}}>The property owner is responsible for accurate declarations and keeping evidence current. HOST review records the evidence checked; it is not a guarantee that hazards cannot exist.</p>{reviewNote&&<p style={{color:"#d49a3f"}}><strong>{ht("HOST review")}:</strong> {reviewNote}</p>}</section>
    {items.map((item,index)=>{const category=categories.find(([key])=>key===item.category);const label=category?.[1]??item.category;const allowsNA=category?.[2]??false;const na=item.applicability==="not_applicable";return <section key={item.category} style={card}><h2 style={{fontSize:19}}>{ht(label)}</h2>
      ...
      <label style={{display:"block",marginTop:12}}>{ht("Reference or explanation")}<input value={item.evidenceReference} onChange={event=>update(index,{evidenceReference:event.target.value})} style={input}/></label>{item.reviewStatus&&item.reviewStatus!=="not_reviewed"&&<p style={{color:"#d49a3f"}}>{ht("HOST review")}: {hStatus(item.reviewStatus)}</p>}
    </section>;})}
