...
async function setupPublishedProperty(suffix: string) {
  ...
  const property = await db.query(`INSERT INTO properties (host_id, name, city, currency, nightly_price, max_guests, status, compliance_status) VALUES ($1, 'HL Test Property', 'Liverpool', 'GBP', 100, 2, 'published', 'approved') RETURNING id`, [hostProfile.rows[0].id]);
